...
const Login = () => {
    ...
    const onSubmit = async(e)=>{
        e.preventDefault()
        try{
            const res = await fetch("https://api.theoriehaast.nl/api/v1/auth/login" , {
                method:"POST",
                headers:{
   ...
   <div className="w-full min-h-[100vh] flex flex-col sm:flex-row sm:justify-center sm:max-h-[100vh]">
    ...
    <h1 className='text-[25px] text-center capitalize text-[#0D67B1] font-bold border-b-2 py-3 sm:text-left px-3'>welcome to . <span>rijinstructeurmakelijkworden  </span></h1>
        ...
        <label htmlFor="" className='w-full flex flex-col gap-3 text-[18px] sm:text-[15px] sm:gap-1'>
            Email
            <input type="text " className='p-2 border-2 bg-[#E8F0FE]' onChange={(e)=>setEmail(e.target.value)} />
        </label>
        <label htmlFor="" className='w-full flex flex-col gap-3 text-[18px]'>
        Wachtwoord
            <input type="text " className='p-2 border-2 bg-[#E8F0FE]' onChange={(e)=>setPassword(e.target.value)} />
        </label>
        <button className="bg-[#FACA00] w-fit p-3 text-[20px] uppercase rounded-xl font-bold mx-auto hover:bg-[#0D67B1] transition-all" type='submit'>Inloggen</button>
