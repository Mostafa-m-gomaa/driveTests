...
const Nav = () => {
    ...
    const logOut = ()=>{
      ...
      PiStackOverflowLogoThin(false)
    }
  
  return (
    <div className="flex bg-[#0D67B1] text-white py-8 px-3 items-center justify-between sm:w-[10%] sm:relative sm:h-[100vh]">
        <Link onClick={()=>setShowMenu(false)} className='capitalize text-[22px] sm:hidden' to="/home">rijinstructeurmakelijkworden      </Link>
    ...
    rounded-md ${showMenu ? "left-0":"-left-[100%]" } sm:relative sm:[&_a]:flex-col-reverse sm:left-0 sm:[&_a]:items-center sm:top-[5%] sm:[&_a]:cursor-pointer sm:gap-5   sm:text-[12px] `}>
        ...
        <Link className='sm:hover:text-[#FADD15] transition-all' onClick={()=>setShowMenu(!showMenu)} to="/home">home <IoIosHome className='sm:text-[45px]' />        </Link>
        ...
        <Link className='sm:hover:text-[#FADD15] transition-all' onClick={()=>setShowMenu(!showMenu)} to="/exam">examens <FaPencilAlt className='sm:text-[45px]' />        </Link>
        <Link className='sm:hover:text-[#FADD15] transition-all' onClick={logOut} to="/">log out <CiLogout className='sm:text-[45px]'/>        </Link>
